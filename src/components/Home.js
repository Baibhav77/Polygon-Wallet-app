...
export default function Home() {
  ...
  const network = magic.network === 'matic' ? 'matic' : 'ethereum';
  ...
  const contract = new web3.eth.Contract(abi, network === "matic" ? maticContractAddress : goerliContractAddress);
  ...
  const handleChangeNetwork = (e) => {
    e.target.value === 'matic' ? setMagic(magicMatic) : setMagic(magicEthereum);
